    ...
    public void buildTooltip(ItemStack itemStack_1, World world_1, List<TextComponent> list_1, TooltipOptions tooltipOptions_1) {
        if (world_1 != null) {
            if (itemStack_1.hasTag()) {
                list_1.add(new StringTextComponent(("Animal Type: " + itemStack_1.getTag().getString("genetics:entitytype"))));
                list_1.add(new StringTextComponent(("Genes: " + Arrays.toString(itemStack_1.getTag().getIntArray("genetics:genes")))));
            } else
